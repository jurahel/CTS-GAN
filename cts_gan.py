...
import numpy as np
from utils_cts_gan import extract_time, rnn_cell, random_generator, batch_generator
...
def timegan(ori_data, parameters):
  ...
  """
    ...
    Z = tf.placeholder(tf.float32, [None, max_seq_len, dim], name="myinput_z")
    X_past = tf.placeholder(tf.float32, [None, max_seq_len, dim], name="myinput_x_past")
    ...
    Args:
      ...
      - T: input time information (sequence length)
    ...
    # Generator
    Gen_In = X_past*Z
    E_hat = generator(Gen_In, T)
    #H_hat = supervisor(E_hat, T)
    #H_hat_supervise = supervisor(H, T)

    # Synthetic data
    X_hat = recovery(E_hat, T)

    # Discriminator
    Y_fake = discriminator(E_hat, T)
    ...
    # 2. Supervised loss
    G_loss_S = tf.losses.mean_squared_error(E_hat, H)# +  10*tf.losses.mean_squared_error(H,E_hat)
    ...
    GS_solver = tf.train.AdamOptimizer().minimize(G_loss_S, var_list=g_vars)
    ...
    saver.save(sess, 'mygan', global_step=1000)
    ...
    for itt in range(iterations):
        # Set mini-batch
        X_mb, T_mb, X_mb_past = batch_generator(ori_data, ori_time, batch_size)
        # Train embedder
        _, step_e_loss = sess.run([E0_solver, E_loss_T0], feed_dict={X: X_mb, T: T_mb, X_past: X_mb_past})
        # Checkpoint
        if itt % 1000 == 0:
    ...
    for itt in range(iterations):
        ...
        # Train generator
        _, step_g_loss_s = sess.run([GS_solver, G_loss_S], feed_dict={X: X_mb, T: T_mb, X_past: X_mb_past, Z: Z_mb})
        # Checkpoint
        if itt % 1000 == 0:
    ...
    for itt in range(iterations):
        ...
        for kk in range(2):
            ...
            _, step_g_loss_u, step_g_loss_s, step_g_loss_v = sess.run([G_solver, G_loss_U, G_loss_S, G_loss_V],
                                                                      feed_dict={Z: Z_mb, X: X_mb, T: T_mb,X_past: X_mb_past,Z:Z_mb})
            # Train embedder
            _, step_e_loss_t0 = sess.run([E_solver, E_loss_T0], feed_dict={Z: Z_mb, X: X_mb, T: T_mb,X_past:X_mb_past,Z:Z_mb})
        ...
        check_d_loss = sess.run(D_loss, feed_dict={X: X_mb, T: T_mb, Z: Z_mb,X_past:X_mb_past})
        # Train discriminator (only when the discriminator does not work well)
        if (check_d_loss > 0.15):
            _, step_d_loss = sess.run([D_solver, D_loss], feed_dict={X: X_mb, T: T_mb, Z: Z_mb,X_past:X_mb_past})
    ...
    X_mb, T_mb, X_mb_past = batch_generator(ori_data, ori_time, no-1)
    # Random vector generation
    Z_mb = random_generator(no-1, z_dim, T_mb, max_seq_len)

    generated_data_curr = sess.run(X_hat, feed_dict={Z: Z_mb, T: T_mb, X_past: X_mb_past})    
    ...
    np.save('ori_data_20200708_60K_H3_min_noise_V2.npy', ori_data)
    np.save('gen_data_20200708_60K_H3_min_noise_V2.npy', generated_data)
